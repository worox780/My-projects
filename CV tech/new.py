...
class Show:
    ...
    def WachTable(self, btn):
        ...
        self.main.destroy()
        Traitement(self.get_name_btn[number])
...
class Test:
    def __init__(self) -> None:
        ...
        self.frame_main = tk.Frame(self.root, relief="groove", bd=1)
    ...
    def Affichage(self):
        for x in range(5):
            for y in range(100):
                tk.Entry(self.frame).grid(row=y, column=x)
        

class Traitement:
    def __init__(self, nom_table:str) -> None:
        self.main = tk.Tk() #fenêtre
        self.main.geometry("+50+50") #position de la fenêtre
        self.main.resizable(False, False) #non redimentionble
        self.name_table = nom_table #nom de la table de la base de donnée
        self.path = r"C:\Users\VERITE\OneDrive\Documents\GitHub\My-project\CV tech\Base.db" #chemin de la base de donnée
        self.entries = [] #liste des entries
        self.link = [] #listes des liens
        self.use = 0 #nombre de changement de la table
        tk.Label(self.main, text="Liste de contact").pack() #label main
        self.cadre = tk.Frame(self.main, height=500) #la frame qui contient le tableur et la scrollbar
        self.cadre.pack() #ajout de la frame
        self.canvas = tk.Canvas(self.cadre) #contient la Frame du tableur
        self.contenu = tk.Frame(self.cadre) #Frame qui contien le tableur
        self.scrollbar = tk.Scrollbar(self.cadre, orient="vertical", command=self.canvas.yview) #la scrollbar qui est dans le cadre, et qui est affiché dans le canvas
        self.canvas.configure(yscrollcommand=self.scrollbar.set)
        self.canvas.pack(side="left")
        self.canvas.create_window((0,0), window=self.contenu, anchor="nw")
        self.contenu.bind("<Configure>",self.myfunction)
        self.TableByTable() #appel pour la génération de la table
        tk.Label(self.main, text="Option").pack() #label
        self.btn_add = tk.Button(self.main, text="Télécharger de nouveau contact", command=self.UploadData) #boutton pour ajouter de la data à la table
        self.btn_add.pack() #ajout du btn
        self.btn_cancel = tk.Button(self.main, text="Quitter la table", command=self.QuitTable) #boutton pour quitter la table
    ...
    def TableByTable(self):
        ...
        for y in range(len(data)): #table generation
            ...
            for x in range(1, len(nom_col)+1):
                if x == 3:
                    e = tk.Button(self.contenu, text=f"lien de la page Linkedin {y}", command=lambda btn=e: self.Link(btn), textvariable=var)
                    e.grid(row=y+1, column=x-1)
                    self.link.append(data[y][x])
                    row_entries.append("")
                else:
                    var = tk.StringVar()
                    e = tk.Entry(self.contenu, width=20, textvariable=var)
                    var.set(data[y][x])
                    e.grid(row=y+1, column=x-1)
                    row_entries.append(var)  #append entry var in a tiny array
            self.entries.append(row_entries)  #append all entry var of a row in an array
